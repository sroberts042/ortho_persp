...
void draw() {
  ...
  if (currentTest != null) {
    renderTestCase();
    if (first) saveFrame("test-" + currentTest.key + ".png");
  }
}
...
void ortho_cube2()
{
  ...
  gtPushMatrix();
  gtTranslate (0.0, 0.0, -4.0);
  gtRotateZ(30 * cos(animTime));
  gtRotateX(30 * cos(animTime));
  gtRotateY(45 * cos(animTime));
  cube();
  gtPopMatrix();
}
